...
| `telegram_wake_words` | No | `""` | Listen mode only. Comma-separated names (e.g. `carson, butler`); a group message whose text, caption or voice transcription contains one as a whole word is treated like an @mention |
| `telegram_group_mode` | No | `mention` | `mention`: in groups, only @mentions and replies reach the agent. `listen`: it reads every group message from allowed users and acts silently, replying only when addressed (needs BotFather privacy mode off). Telegram progress messages and tool hints are turned off in this mode. Reply protocol: a reply of exactly `NOTED` becomes a 👌 reaction, `NO_REPLY` sends nothing; overheard group messages stay silent unless the reply is `NOTED` or starts with `SAY:`. To post into a forum topic, the `message` tool takes `chat_id` `<chat_id>:topic:<thread_id>` (a bare group id posts in General); malformed Telegram targets return an error instead of being reported as sent |
| `show_system_messages` | No | `false` | Post system notices such as "Compressing context…" / "Context compacted." in chat channels. The WebUI always shows them |
